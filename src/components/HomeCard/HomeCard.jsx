...
const HomeCard = ({card}) => {
  ...
  <figure className="overflow-hidden">
     ...
     <Link to={`/app/${id}`}><img src={image} alt="" className="w-full object-cover justify-center items-center p-10" /></Link>
  ...
  <div className="card-body">
    ...
    <div className="card-actions justify-between">
      <button className="btn text-[#00D390] "><img src={icon1} alt="" className="w-4" />{downloads}M</button>
      <button className="btn text-[#FF8811] "><img src={icon2} alt="" className="w-4" /> {ratingAvg}</button>
    </div>
  </div>
</div>
      </div>
